...
app.use(cors({ 
    origin: "http://localhost:3000", // อนุญาตให้ React เข้าถึง 
...
app.post("/login", (req, res) => { 
    ...
    db.query("SELECT * FROM Customer WHERE Email = ?", [email], (err, result) => { 
        ...
        const hashedPassword = user. Password; // ตรวจสอบว่ารหัสผ่านถูกต้องหรือไม่ 
    ...
    // สร้าง JWT Token   
        ...
        res.json({ message: "Login successful", token,id: user.CustomerID}); 
    }); 
});
